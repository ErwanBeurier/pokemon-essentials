...
def followingMoveRoute(commands,waitComplete=false)
  return if !$Trainer.scGetFollowing || !$PokemonGlobal.followerToggled
...
def pbToggleFollowingPokemon(forced = nil,anim = true)
  ...
  return if !$Trainer.scGetFollowing
  ...
  if $PokemonGlobal.followerToggled
    ...
    ret = Events.FollowerRefresh.trigger($Trainer.scGetFollowing)
...
def pbPokemonFollow(x)
  return false if !$Trainer.scGetFollowing
...
def pbTalkToFollower
  ...
  firstPkmn = $Trainer.scGetFollowing
  pbPlayCry(firstPkmn)
  event = pbGetDependency("FollowerPkmn")
  randomVal = rand(6)
...
def pbPokemonFound(item,quantity = 1,message = "")
  ...
  pokename = $Trainer.scGetFollowing.name
...
#-------------------------------------------------------------------------------
  def add_following_time
    ...
    $Trainer.scGetFollowing.happiness += 1 if ($PokemonGlobal.timeTaken % 5000) == 0
    $PokemonGlobal.followerHoldItem = true if ($PokemonGlobal.timeTaken > 15000)
  end

  def removeAllButFollower(name)
...
def pbHiddenMoveAnimation(pokemon,followAnim = true)
  ...
  if ret && followAnim && $PokemonTemp.dependentEvents.refresh_sprite(false,true) && pokemon == $Trainer.scGetFollowing
