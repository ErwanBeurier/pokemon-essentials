...
class Pokepoker
  ...
  def initialize(fee,blindvalue, opponent_names = nil)
    @chipsvariable = 68
    @randomnpcnames = true
    @deck = [
    ...
    elsif opponent_names.is_a?(Array) && opponent_names.length >= 3
      @players[1][3] = opponent_names[0]
      @players[2][3] = opponent_names[1]
      @players[3][3] = opponent_names[2]
    end
    @players[0][0] += @fee
    @players[1][0] += @fee
    @players[2][0] += @fee
...
PluginManager.register({
  :name => "Poke Poker",
  :version => "1.0",
  :credits => ["Aioros","AmirX3"],
  :link => "https://www.pokecommunity.com/showthread.php?t=439142"
})
